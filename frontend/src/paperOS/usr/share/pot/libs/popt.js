//# sourceURL=usr/share/pot/libs/popt.js 


/**
 *---------------ABOUT POPT---------------
 * POPT handles all of the keyboard inputs and displays
 * it manages where things are written to the screen,
 * alongside how inputs are managed. This contains stuff such
 * as the number of lines, the position of the cursor,
 * and more. But, it does not know what you are doing exactly.
 */

/**
 *@property {HTMLElement} terminalElem the selected element to be the host for the terminal
  @property {number} rows the number of rows in characters the POPT object is 
  @property {number} columns the number of columns in characters the POPT object is 
...
class POPT {
    ...
    */
    constructor(element, rows = 47, columns = 100) {
        ...
        meas.innerText = "M"; // use a representative character
        ...
        this.terminalElem.style.top = "0";
        this.terminalElem.style.left = "0";
    ...
    */
    getLine(number) {
        return this.terminalElem.children[number];
    }
    /**
    ...
    scroll(amount = 1) {
        for (let i = 0; i < amount; i++) {
            this.terminalElem.children[i].remove();
            let row = document.createElement("span");
            row.style.whiteSpace = "pre";
            row.style.height = `${this.rowSize}px`;
            row.style.width = `${this.columnSize * this.columns.value}px`;
    ...
    */
    say(text) {
        //clean up the inputs EXCEPT for spans (for custom colors and stuff) (not like they can't already make applications... might remove later?)
        let lines = text.replace("<", "&lt;").replace(">","&gt;").replace("&lt;span&gt;", "<span>").replace("&lt;/span&gt;", "</span>");
        //splitting it up to an array
        lines = text.split("\n")

        //displays the first line (if the first line isn't nothing/there is a newline first)
